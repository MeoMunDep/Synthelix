...
if not exist configs.json (
    ...
    echo   "referralCode": ["kqrf5FWN"]>> configs.json
    echo }>> configs.json
...
if exist "..\node_modules" (
    ...
    cd ..
    CALL npm install user-agents axios colors https-proxy-agent socks-proxy-agent ethers web3 tough-cookie axios-cookiejar-support
    cd %~dp0
) else (
